...
# -*- coding: utf-8 -*-
"""
Script for loading XArm 6 robot using robot_descriptions package
"""
...
def load_xarm6_with_robot_descriptions():
    """Load XArm 6 robot using robot_descriptions package"""
    try:
        from robot_descriptions.loaders.mujoco import load_robot_description
        
        print("Loading XArm 6 robot using robot_descriptions...")
        
        # Try to load XArm 6 robot
        try:
            model = load_robot_description("xarm6_mj_description")
            print("✓ Successfully loaded XArm 6 robot using robot_descriptions!")
            return model
        except Exception as e:
            print("XArm 6 not found in robot_descriptions, trying other methods...")
...
def load_xarm6_from_menagerie():
    """Load XArm 6 robot from MuJoCo Menagerie"""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Go up one directory to reach the menagerie root
    menagerie_dir = os.path.dirname(current_dir)
    xarm6_path = os.path.join(menagerie_dir, "ufactory_xarm6", "xarm6_nohand.xml")
    
    if os.path.exists(xarm6_path):
        print("Loading XArm 6 robot from MuJoCo Menagerie...")
        model = mujoco.MjModel.from_xml_path(xarm6_path)
        print("✓ Successfully loaded XArm 6 robot from Menagerie!")
        return model
    else:
        print("Error: XArm 6 model file not found")
        return None

def explore_xarm6_model(model):
    """Explore XArm 6 robot model"""
    if model is None:
        return
    
    print("\n" + "="*50)
    print("XArm 6 Robot Model Information")
    print("="*50)
...
def test_xarm6_movement(model):
    """Test XArm 6 robot movement"""
    if model is None:
        return
    
    print("\n" + "="*50)
    print("Testing XArm 6 Robot Movement")
    print("="*50)
    
    # Create data object
    data = mujoco.MjData(model)
    
    # Define test positions for XArm 6
    test_positions = [
...
def get_xarm6_specifications():
    """Get XArm 6 specifications"""
    specs = {
        'max_payload': '5kg',
...
def show_xarm6_specifications():
    """Show XArm 6 specifications"""
    specs = get_xarm6_specifications()
    print("\n" + "="*50)
    print("XArm 6 Robot Specifications")
    print("="*50)
...
def main():
    ...
    model = load_xarm6_with_robot_descriptions()
    
    # If failed, load from Menagerie
    if model is None:
        model = load_xarm6_from_menagerie()
    
    if model is not None:
        # Explore model
        explore_xarm6_model(model)
        
        # Test movement
        test_xarm6_movement(model)
        
        # Show specifications
        show_xarm6_specifications()
        
        print("\n" + "="*50)
        print("XArm 6 model loaded successfully!")
        print("To start interactive viewer, run:")
        print("python -m mujoco.viewer --mjcf ../ufactory_xarm6/scene.xml")
        print("="*50)
    else:
        print("Failed to load XArm 6 robot model")
